...
detect_swig_php_flag() {
  SWIG_VER="$($(which swig) -version 2>&1 | awk '/SWIG Version/{print $3}')"
...
# Replace the ./libtool script generated by config.status with a clean wrapper
# around the real system libtool binary.  The generated script is produced from
# an ancient ltmain.sh bundled with FreeSWITCH that pre-dates --tag support and
# whose argument-parsing loop is incompatible with a modern phpize-generated
# Makefile.  The system libtool (apt package libtool, already installed) is
# fully modern; it only needs --tag=CXX stripped because the FreeSWITCH
# configure.ac does not register the CXX tag.
...
replace_libtool_with_wrapper() {
  ...
  # Locate the real system libtool binary (not a generated script)
  _sys_lt=""
  ...
  if [ -z "$_sys_lt" ]; then
    print_warn "System libtool binary not found -- cannot replace, falling back to in-place patch."
  ...
  # Write a self-contained wrapper. Use printf so no heredoc quoting issues.
  printf '%s\n' '#!/bin/sh' \
    '# Wrapper: strips --tag=* then delegates to the real system libtool.' \
    "REAL_LIBTOOL='${_sys_lt}'" \
    'args=""' \
    'for a in "$@"; do' \
    '  case "$a" in' \
    '    --tag=*) ;;' \
    ...
    'exec "$REAL_LIBTOOL" $args' \
    > "$_lt"
  chmod +x "$_lt"
  print_success "Generated libtool replaced with system wrapper."
}

# Fallback in-place patch used only when the system libtool binary is absent.
_tag_strip_patch() {
  _lt="$1"
  if [ ! -f "$_lt" ]; then
  ...
  printf '%s\n' \
    ...
    '_p_i=0; _p_total=$#' \
    'while [ "$__p_i" -lt "$__p_total" ]; do' \
    '  _p_a="$1"; shift; _p_i=$((_p_i + 1))' \
    '  case "$__p_a" in --tag=*) ;; *) set -- "$@" "$__p_a" ;; esac' \
    'done' \
    'unset _p_a _p_i _p_total' \
