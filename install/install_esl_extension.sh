...
# Patch the ./libtool script generated by config.status so it silently strips
# the --tag=CXX argument that make passes when compiling C++ sources.
# The old ltmain.sh bundled with FreeSWITCH does not understand --tag at all,
# which causes it to die before it even tries to compile anything.
# We call this function AFTER ./configure (so config.status has already run
# and written ./libtool) and BEFORE make (so nothing overwrites our patch).
patch_generated_libtool() {
  _lt="$1"
  if [ ! -f "$_lt" ]; then
...
M4EOF
  ...
  # Patch the ./libtool that config.status just wrote BEFORE make runs
